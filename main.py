...
pipeline_path = r'pipeline_model\german_model_grad_tree.pkl'
with open(pipeline_path, 'rb') as file:
    pipeline = pkl.load(file)

# dataset, hard coded for now
dataset_path = r'pipeline_model\german_test.csv'
dataset = pd.read_csv(dataset_path)
dataset.rename(columns={"Unnamed: 0": "ID"}, inplace=True) # HARD CODED

# extracting the dataset name
file_name = os.path.basename(dataset_path)  # Extracts "filename.csv"
dataset_name = os.path.splitext(file_name)[0]  # Extracts "filename"
...
user_query = 'Are people younger than 25 vulnerable to bad credit risk?'
...
before_dataset, shap_values_before, after_dataset, shap_values_after = calc_SHAP(transformations, model, dataset, dataset_name, update_query, retrieve_query)

# getting final predictions and aggregate statistics
final_dataset_before, summary_stats_before, final_dataset_after, summary_stats_after = pred_and_stats(pipeline, before_dataset, after_dataset)
...
response = ml_to_natural_language(shap_values_before, shap_values_after, feature_names, user_query, filter_representation, final_dataset_before, final_dataset_after, importance_table_before, importance_table_after, summary_stats_after, openai_api_key)
print('---------------------------------')
print(response)